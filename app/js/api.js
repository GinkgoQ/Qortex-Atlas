...
export const Api = {
  ...
  evaluateCoverageDesign: (id, snapshot, body) => req(`/dataset/${id}/coverage/evaluate-design${qs({ snapshot })}`, { method: 'POST', body: JSON.stringify(body) }),
  fmriQc: (id, params = {}) => req(`/dataset/${id}/fmri-qc${qs(params)}`),
  startFmriQcRun: (id, snapshot, body = {}) => req(`/dataset/${id}/fmri-qc/runs${qs({ snapshot })}`, { method: 'POST', body: JSON.stringify(body) }),
  fmriQcArtifactUrl: (runId, artifact) => `${BASE}/fmri-qc/runs/${encodeURIComponent(runId)}/artifacts/${encodeURIComponent(artifact)}`,
  signalAnalysis: (id, params = {}) => req(`/dataset/${id}/signal-analysis${qs(params)}`),
  startPublicRoiConnectivity: (body = {}) => req('/analysis/roi-connectivity/validate-public', { method: 'POST', body: JSON.stringify(body) }),
  publicRoiConnectivityArtifactUrl: (runId, artifact) => `${BASE}/analysis/roi-connectivity/runs/${encodeURIComponent(runId)}/artifacts/${encodeURIComponent(artifact)}`,
  ...
  annotations: (id, { snapshot, sourcePath } = {}) => req(`/dataset/${id}/annotations${qs({ snapshot, source_path: sourcePath })}`),
  saveAnnotation: (id, snapshot, body) => req(`/dataset/${id}/annotations${qs({ snapshot })}`, { method: 'POST', body: JSON.stringify(body) }),
  annotation: (id, annotationId, { snapshot, revision } = {}) => req(`/dataset/${id}/annotations/${encodeURIComponent(annotationId)}${qs({ snapshot, revision })}`),
